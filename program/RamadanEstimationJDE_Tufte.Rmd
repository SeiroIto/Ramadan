...
## Summary statitistics and simple DID
...
Main results in text. For full results, see <a name = "ResultsTableMain">[the full table in appendix.](#FullResultsMain)</a>
...
Overall, agricultural households are poorer, and nonattending children claim that financial as the reason of not going to school.  
...
write.tablev(latextab(as.matrix(reatab),
  ...
  hcenter = c(2, 2.5, rep(1, ncol(reatab)-2)), 
...
write.tablev(latextab(as.matrix(reatab),
  ...
  hright = c("", rep("$", ncol(reatab)-1)), 
  headercolor = NULL, delimiterline = NULL), 
  "../draft/Tables/reasons.tex", colnamestrue = F)
...
kt <- kable(reatab, align = paste0("r", paste0(rep("c", ncol(reatab)-1), collapse = "")), 
  caption =   "Reasons stop going to school by consumption quartile")
...
ktReaPC4 <- footnote(kt, 
           general = "Sample of direct offspring of household heads. Observations are children who are not attending school in each year. Dropouts are children who was attending school in 1999 and disconitnue by 2002. Nonattendees are all children who are not attending school in respective years."
           )
...
reatab <- data.frame(cbind(
  Group = rep(paste0("\\mbox{\\scriptsize ", 
    c("Non-attdendees 1999","Non-attdendees 2002","Dropouts 2002"), "}"), 2), 
  formatC(reatab[, -ncol(reatab)], digits = 2, format = "f"), total = reatab[, ncol(reatab)]))
...
write.tablev(latextab(as.matrix(reatab),
  ...
  hright = c("", rep("$", ncol(reatab)-1)), 
  headercolor = NULL, delimiterline = NULL), 
  "../draft/Tables/reasons_ag.tex", colnamestrue = F)
#### HTML table
reatab <- data.table(reatab)
...
* The rate of financial is decreasinng in consumption quartile within each group.  

```{r reasons of not going to school bg ag nonag, echo = F}
kt
```

* Financial among nonattending children of agricultural housesholds is greater than non-agricultural households.  
...
#### LaTeX table
write.tablev(latextab(as.matrix(agp), 
...
write.tablev(latextab(as.matrix(agp), 
  ...
  hright = c("", rep("$", ncol(agp)-1)), 
  headercolor = NULL, delimiterline = NULL), 
  "../draft/Tables/tab_agpc4.tex", colnamestrue = F)
#### HTML table
...
* Agricultural households are more concentrated up to the 3rd quartile.  
...
<details><summary>Click here to see the R script of enrollmen by age and age at enrollment.</summary>
```{r enrollment by age}
...
```{r enrollment figure1, fig.cap='Enrollment rates by age', echo = F}
g1
```
```{r enrollment figure2, echo = F, fig.cap='Age at class 1 enrollment'}
g2
```
