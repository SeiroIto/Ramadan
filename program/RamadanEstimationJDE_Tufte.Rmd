...
Definition of `agHH' is created in `Create2RoundPanel.rnw`. `e00` is individual level data with member occupation and income source. 
...
boxsize <- .85
library(qs)
zi00N <- qread("../save/DID2024_N_MainResults_old.qs")
...
zm <- Zm[, .(survey, thana, hh, uniquid, hdagHH, isagHH, 
  sd, AgeIn1999, AgeIn2006, schoolp, program, anyprog)]
zm[, aghh := as.integer(hdagHH>0)]
zm[, y1999 := as.integer(survey==1999)]
zm[, StipendProgram := as.integer(grepl("stip", anyprog))]
#### In 2002, progam = StipendProgram except for 6 cases, 
#### so they are synonymous in 2002 but not in 1999. Need to use "program".
#### Changes in program: Even among 9-12 years (most likely enrollers), 
#### 1/2 change their recepient status between 1999 and 2002
cat("Stipend program recepients amonng 9-12 in 1999:\n")
addmargins(table(zm[uniquid %in% 
  uniquid[AgeIn1999 >= 9 & AgeIn1999 <= 12], 
  .(aghh, st = paste0(StipendProgram, collapse = "")), by = uniquid][, .(aghh, st)]))
...
clusterNum2 <- clusterNum1 <- clusterNum0
...
source("../program/RegressorPieces.R")
#### Why we use regressorsM2024Sep24 instead of regressorsM2024Feb23
#### program should use "prog" piece (program) because non-stipend programs 
#### mostly disappear after 1999, and stipend program recepients vary 
#### a lot between years. program variable captures changes in stipend 
#### program recipients and disappearance of non-stipend programs. 
#### By using program and not its components, we stay agnostic about 
#### selecting into/out of programs.
...
regressorsM2024Apr10 <- 
  c("^\\(Interce", 
    ...
    paste(indivnoagesib2, hdedunoagHH2, spedunoagHH2, hygiene2, sep = "|"))
regressorsM2024 <- 
  regressorsM2024Sep24 <- 
  c("^\\(Interce", 
    paste(indivonlyage, yield, weather.variables1, prog, area2, sep = "|"),
    paste(indivnoagesib2, hdedunoagHH2, spedunoagHH2, hygiene2, sep = "|"))
...
regressorsP2024Apr10 <- 
  c("^\\(Interce", 
    ...
    paste(indivnoagesib3, hdedunoagHH3, spedunoagHH3, hygiene3, sep = "|"))
regressorsP2024 <- 
regressorsP2024Sep24 <- 
  c("^\\(Interce", 
    paste(indivonlyage, yield, weather.variables1, prog, area3, sep = "|"),
    paste(indivnoagesib3, hdedunoagHH3, spedunoagHH3, hygiene3, sep = "|"))
```
...
ltb4 <- ltb[
    c(1:(aghhrows[1]+8), r2rows[1]+c(0, 2:7),
       aghhrows[2]+(-2:8), r2rows[2]+c(0, 2:7), length(ltb)-4:0)
    ]
write.tablev(ltb4
  ,  paste0("../save/", c("Main", "Placebo")[ii], "Older", (10:12)[s], 
...
```{r show days absent results, echo = F, eval = F}
####resultsG[[ii]][[jj]][[en]][[s]][[j]][[ge]][[m]][[clnum]][[k]]
####resultsD[[ii]][[jj]][[s]][[j]][[ge]][[m]][[clnum]][[k]]
ii <- jj <- en <- s <- 1
j <- clnum <- 2
...
write.tablev(TabAttrit,  
  "../save/App_AttritionEstimation.tex"
  , colnamestrue = F)
write.tablev(TabAttrit,  
  "../draft/Tables/App_AttritionEstimation.tex"
  , colnamestrue = F)
#### HTML table
for (m in c(1, 3)) {
...
z1[, .(Enroll = mean(schoolp, na.rm = T), N = .N), 
  by = .(AgeIn1999, agHH0>0)][order(AgeIn1999, agHH0), ]
pathsource00 <- paste0(pathsource, "/ffe/2000/Data/HouseholdData/STATA/")
fn <- list.files(pathsource00)
fn0 <- list.files(pathsource00, full.names = T)
#### 1.A.1 section (roster)
