...
```{r program random test, echo = T, eval = T, class.output="scroll-100"}
...
setnames(z.1, "LHS", "Enrolled")
setnames(z01, "LHS", "Enrolled")
setnames(z.0, "LHS", "Enrolled")
...
#### This one is for JR and JDE
reo <- paste0("Enr|[Aa]ge|Sex|sex|d ed|e ed|pri|seco|sibl|hold|asse|wat|toi|latr|Mus|",
  "[pP]rog|rain|high|low|[Yy]i|Flo")
reo <- unlist(strsplit(reo, "\\|"))
reo2 <- NULL
for (rr in reo) reo2 <- c(reo2, grep(rr, unlist(SampleDiff[, 3])))
reo2 <- c(t(cbind(reo2, reo2+1)))
SampleDif <- SampleDiff[reo2, ]
varlabels <- 
c(
"Enrollment status of the child", 
"Age of the child", 
"Sex of the child (female = 1)", 
"HH head sex (female = 1)", 
"HH head education: primary", 
"HH head spouse education: primary", 
"HH head education: secondary", 
"HH head spouse education: secondary", 
"Number of older brothers", 
"Number of older sisters", 
"Per-member land holding (decimal)", 
"Per-member nonland asset (1000 Tk)", 
"HH has water supply (piped water)", 
"HH has sanitary latrine (Structured toilet)", 
"HH is non-Muslim", 
"HH receives Food-for-Education (FFE) program", 
"HH receives other non-FFE program",
"HH receives a program",
"Rainfall (in millimeters)",
"High temperature (in Celsius)",
"Low temperature (in Celsius)",
"Paddy yield (in '000 metric tons)",
"Flooded (=1)"
)
SampleDif[seq(1, nrow(SampleDif), 2), 3] <- varlabels
SDif.print = copy(SampleDif)
for (j in c("overall", "agHH", "nonagHH", "t", "Satterthwaite")) 
  SDif.print[, (j) := formatC(eval(parse(text=j)), digits = 4, format = "f")]
SDif.print <- rbind(as.matrix(SDif.print), 
  c("", "", "n", nrow(z01), nrow(z.1[tee == 1, ]), nrow(z.0[tee == 1, ]), "", ""))
SDif.print <- a2b(SDif.print, "   NA", "")
iispace <- which(
  SDif.print[, 3]=="" & 
    !grepl("interaction with|^n$|bar.R|thana dum|mean at|raw DID", SDif.print[, 3])
    )
iispace <- iispace-1
iispace2 <- iispace[seq(2, length(iispace), 2)]
for (kk in c("overall", "agHH", "nonagHH")) 
  SDif.print[iispace+1, kk] <- 
    paste0("(", 
      formatC(as.numeric(SDif.print[iispace+1, kk]), digits = 3, format = "f")
    , ")")
for (kk in c("t", "Satterthwaite")) 
  SDif.print[iispace, kk] <- 
    paste0("[", 
      formatC(as.numeric(SDif.print[iispace, kk])*100, digits = 2, format = "f")
    , "]")
rm(zi00)
SDltx <- SDif.print[, -(1:2)]
colnames(SDltx) <- c("Individual and household level variables", "Overall", "Agri HH", "Non-agri HH",
  "t-test", "Satterthwaite")
SepCols <- 3 
ltb <- latextab(SDltx, delimiterline = NULL, 
    hcenter = c(6, rep(1.3, ncol(SDltx)-1)),
    hleft = c("\\scriptsize", rep("\\hfil\\scriptsize$", ncol(SDltx)-1)), 
    hright = c("\\hfill", rep("$", ncol(SDltx)-1)),
    headercolor = NULL, 
    adjustlineskip = "-.5ex", adjlskiprows = grep("\\w\\w", SDltx[, 1]),
...
aghhrows <- grep("Rain", ltb)
###ltb[length(ltb)-1] <- gsub("\\\\\\\\", "", ltb[length(ltb)-1])
ltb <- c(
  ltb[1], "\\hline",
  ltb[2:3], "\\line",
  ltb[4:(aghhrows-1)],
  "\\hline\\multicolumn{7}{l}{\\scriptsize Thana level variables}\\\\\\hline",
  ltb[aghhrows:(length(ltb)-1)], "\\hline",
  ltb[length(ltb)]
  )
ltb <- ltb[!grepl("Raw", ltb)]
write.tablev(ltb
  ,  "../draft/Tables/AgNonagDestat.tex"
  ,  colnamestrue = F)
```
</details>
...
```{r find the difference in JHR and current estimation data, eval = F}
ii <- jj <-  m <- s <- de <- 1
j <- clnum <- 2; ge <- 3
...
```
```{r current and JHR results 1018 nuclear all, eval = F}
 rbindlist(list(cisj, cis))
```
<!-- Results of 10 - 18, nuclear, boys.  -->
```{r Show results for main 10 - 18 nuclear boys, eval = F}
ii <- jj <-  m <- s <- 1
j <- clnum <- 2 ## extended HH, Satterthwaite
ge <- 3  # boys+girls
...
```{r Show results for main 10 - 18 nuclear girls, eval = F}
ii <- jj <-  m <- s <- 1
j <- clnum <- 2 ## extended HH, Satterthwaite
ge <- 3  # boys+girls
...
<a name = "ResultsTableMain">Main results in text.<a/>
...
for (ge in c(3, 1, 2)){
  ...
  for (m in c(2)) {
    ...
    tab0 <- tabs2latex4(cis, informat = "sattCI", outformat = "epc", xx.yyy = T,
      AddStarIntabstarP = ADDStar, CIIntinysize = T, UseHTML = T)
...
kt <- row_spec(kt, 0, align = paste0(rep("c", ncol(Tab)-1), collapse = ""), 
  background = bgcolor)
kt <- row_spec(kt, grep("Agri", Tab[, "variables"])+0:2, bold = F, 
...
See [Appendix full table](#FullResultsMain) for the full results.   
...
for (jj in 1:2) {
  ...
  for (ge in c(3, 1, 2)) {
    ...
    gg <- which(ge==c(3, 1, 2))
    Tab00 <- rbind((jj-1)*9+(gg-1)*3+1:ncol(Tab00), Tab00) 
    Tab0 <- cbind(Tab0, Tab00)
  }
  rn <- rownames(tab0)
  #rn[grepl("^CI|^p", rn)] <- ""
  rn2 <- c("R2", "N: agHH", "N", 
...
ltb <- c(
  ...
  ltb[(aghhrows[1]-1):(aghhrows[2]-4)],
  "\\multicolumn{12}{l}{\\scriptsize Common specifications}\\\\",
  ltb[(aghhrows[2]-3):(aghhrows[2]-2)],
...
kt <- pack_rows(kt, group_label="1999 cohort", aghhrows[2]-1, grep("Co.*ana", Tab[, 1])[2]-2)
...
#### results[[ii]][[jj]][[s]][[j]][[ge]][[m]][[clnum]][[k]] 
nR0 <- unique(NRFM[agelb == (10:12)[1] & grepl("is", agdef) & grepl("^d", HHtype), ])
enr00 <- EnrFM[agelb == (10:12)[1] & grepl("B", inference) & 
  grepl("is", agdef) & grepl("^d", HHtype), ]
...
ltb4 <- ltb[
    c(1:(aghhrows[1]+8), r2rows[1]+c(0, 2, 3:7),
       aghhrows[2]+(-2:8), r2rows[2]+c(0, 2, 3:7), length(ltb)-4:0)
    ]
write.tablev(ltb4
  ,  paste0("../save/", c("Main", "Placebo")[ii], "Older", (10:12)[s], 
...
* Non-Muslim boys seem to reduce enrollment rates after the Ramadan-harvest overlap ends, which is the oppositte of festivity or fasting related confounder would suggest. But estimates are imprecise and nothing conclusive can be drawn.  
* Girls in flooded areas decrease enrollment rates, both in agricultural and non-agricultural househlds.  
  ...
  for (gd in 1:4) {
    ...
    # 2 months of planting season absenteeism in a 3 month recall question.
      ...
      for (ge in c(3, 1, 2)) {
        ...
        assign(
            paste0(c("M", "P")[ii], c("NIP", "NCP", "DCP", "DCS")[gd], c("b", "g", "bg")[ge], c(9, 2)[yy]) 
      ...
      assign(
          paste0(c("M", "P")[ii], c("NIP", "NCP", "DCP", "DCS")[gd], c(9, 2)[yy]) 
        , Tab000)
      Tab00 <- rbind(Tab00, Tab000)
    } # yy
    assign(
        paste0(c("M", "P")[ii], c("NIP", "NCP", "DCP", "DCS")[gd]) 
      , Tab00)
...
#### paste0(c("M", "P")[ii], c("NIP", "NCP", "DCP", "DCS")[gd], c("b", "g", "bg")[ge], c(9, 2)[yy]) 
row1 <- MNIPbg9 # Main NumGrade incomplete panel boys+girls 1999
setnames(row1, as.character(1:ncol(row1)))
row1 <- data.table(rbind(1:3, as.matrix(row1)))
row2 <- cbind(MNCPbg9, MDCPbg9) # Main NumGrade DaysAbsent comp panel boys+girls 1999, 2002
setnames(row2, as.character(1:ncol(row2)))
row2 <- data.table(rbind(3+1:ncol(row2), as.matrix(row2)))
row3 <- MDCSbg9 # Main DaysAbsent cross section boys+girls 1999
setnames(row3, as.character(3+1:ncol(row3)))
row3 <- data.table(rbind(9+1:ncol(row3), as.matrix(row3)))
row4 <- MDCSbg2 # Main DaysAbsent cross section boys+girls 2002
...
MNDbg <- MNDbg[-(rep(iii, each = 3)+0:2), ]
MNDbg <- rbind(MNDbg, 
...
#### Add * year 2002 to panel estimates (first 2)
aghhrows <- grep("Agri", ltb)
for (iii in 1:2)
  ltb[aghhrows[iii]] <- gsub("household", "household * year 2002", ltb[aghhrows[iii]])
ltb <- gsub("CI\\$.*\\$", "", ltb)
ltb <- gsub("p\\$.*\\$", "", ltb)
ltb <- gsub("R2", "$\\\\bar{R}^{2}$", ltb)
ltb <- gsub("(^\\\\cline.*$)", "\\1[-1ex]", ltb)
...
kt <- row_spec(kt, rep(aghhrows, each = 3)+0:2, 
  color = "black", background = "#87cefa")
kt <- pack_rows(kt, group_label="Students enrolled in 1999", 
  aghhrows[1]-1, aghhrows[2]-1)
kt <- pack_rows(kt, group_label="Students enrolled in 1999 and 2002", 
  aghhrows[2]-1, aghhrows[3]-1)
kt <- pack_rows(kt, group_label="Students enrolled in 1999 and 2002, cross section OLS of 2000", 
  aghhrows[3]-1, aghhrows[4]-1)
kt <- pack_rows(kt, group_label="Students enrolled in 1999 and 2002, cross section OLS of 2003", 
  aghhrows[4]-1, grep("Cov.*ana", Tab[, 1])-1)
...
* Grade progression impact estimates are negative. Exam-harvest overlap delays progression by .21-32 years (`r round((c(.21, .32))*12, 2)` months).   
...
* What this means: Even when the children of agricultural households are not dropping out of school, at least a quarter of them lag behind in one grade progression relative to the children of non-agricultural households.  
* Point estimates are larger in magnitude for boys (see [Appendix table](#NumGradeDaysAbsentResults)).  
* Absence increases for about .68-.98 day per month.  
...
* Cross sectional estimates in 2003 (using enrollers of 1999 and 2002) show a monthly .70 - .83 day increase for children of agricultural households. This reflects that the survey asks days absent of June - August of 2003, which coincide with July-August planting period.  
...
See [a more concise table](#ResultsTableMain) in main text.

```{r ShowMainResults, eval = T, warning = F}
##### results <- qread("../save/DID2024_MainResults.qs")
ii <- jj <-   s <- de <- 1
j <- m <- clnum <- 2; ge <- 3
...
kt <- row_spec(kt, grep("^agHH.yr.$", rownames(tab))+0:2+1, bold = F, 
  color = "black", background = "#87cefa")
kt <- pack_rows(kt, group_label="Time varying covariates", 
  grep("^\\(Interce", rownames(tabmat))+3, grep("Agr", rownames(tabmat))-1)
kt <- pack_rows(kt, group_label="Interaction with year 2002", 
  grep("Agr", rownames(tabmat)), grep("sex", rownames(tabmat))[2]-1)
kt <- pack_rows(kt, group_label="Interaction with agHH*year 2002", 
  grep("sex", rownames(tabmat))[2], grep("Thana", rownames(tabmat)))
kt <- scroll_box(kt, height = "800px")
kt <- footnote(kt, 
...
```{r show placebo 2002 cohort results, eval = T}
jj <-  s <- de <- 1
ii <- j <- m <- clnum <- 2
Tab <- NULL
for (ge in c(3, 1, 2)) {
...
kt <- row_spec(kt, 0, align = paste0(rep("c", ncol(Tab)-1), collapse = ""), 
  background = "#ffcc99")
kt <- row_spec(kt, rep(grep("^agHH.yr", rownames(Tab)), each = 3)+rep(0:2, 3), bold = F, 
  color = "black", background = "#87cefa")
aghhrows <- grep("^.Intercept", rn)
#### Grouping rows
kt <- pack_rows(kt, group_label="Boys+girls", aghhrows[1], aghhrows[2]-1)
kt <- pack_rows(kt, group_label="Boys", aghhrows[2], aghhrows[3]-1)
kt <- pack_rows(kt, group_label="Girls", aghhrows[3], grep("^Tha", Tab[, 1])-1)
...
```{r show placebo 1999 cohort results, eval = T}
 s <- de <- 1
ii <- jj <- j <- m <- clnum <- 2
Tab <- NULL
for (ge in c(3, 1, 2)) {
...
kt <- scroll_box(kt, height = "800px")
kt <- footnote(kt, 
           general = "Sample of 1999 cohort, direct offspring of household heads. ",
           number = c(SpecMemo1html, SEMemoForSelectedResultsNoStarhtml)
           )
kt
```
...
for (ii in 1:2) {
  ...
  resl <- Res[grepl(agyr, Coef) & grepl(c("main", "placebo")[ii], objective) 
    & grepl("di", HHtype), 
    .(Coef, data, gender, agdef, Agdef, agelb, reg, HHtype, inference, beta, p, CI_L, CI_U)][
  ...
  for (jj in 1:jmax) {
    ...
    for (s in 1:3) {
      ...
      SepCols <- c(3, 6) 
      ltb <- latextab(mrtab, delimiterline = NULL, 
          hcenter = c(3.25, rep(1.5, ncol(mrtab)-1)),
          hleft = c("\\scriptsize", rep("\\hfil\\scriptsize$", ncol(mrtab)-1)), 
          hright = c("\\hfill", rep("$", ncol(mrtab)-1)),
          headercolor = NULL, 
      ...
      if (ii == 1 & s == 1) {
        write.tablev(ltb1
          ,  paste0("../draft/Tables/", c("Main", "Placebo")[ii], c(1999, 2002)[jj], "Older", (10:12)[s], 
               "ByGenderByAgHHdefResults1.tex")
          ,  colnamestrue = F)
        write.tablev(ltb2
          ,  paste0("../draft/Tables/", c("Main", "Placebo")[ii], c(1999, 2002)[jj], "Older", (10:12)[s], 
      ...
      for (iii in 1:length(aghhrows)) 
        kt <- pack_rows(kt, group_label=Aghh.defs2[iii], aghhrows[iii]-1, aghhrows[iii]+2)
...
cis[, Agdef := factor(Agdef, levels = Aghh.defs[c(2:4, 1)])]
...
cis[, yintercept := 0]
cis[, gender := factor(gender, levels = genderitems)]
library(ggplot2)
...
<details><summary>Click here to see the R script of plotting estimates on older siblings.</summary>

```{r plot main older siblings, eval = T, echo = T}
#### results <- qread("../save/DID2024_MainResults.qs")
#### results <- qread("save/DID2024_MainResults.qs")
s <- de <- 1
j <- m <- clnum <- 2
...
cis[, ageLB := factor(ageLB)]
cis[, cohort := factor(cohort)]
cis[, spec := factor(spec)]
cis[, yintercept := 0]
cis[, gender := factor(gender, levels = genderitems)]
cis[, Coef := factor(Coef)]
cis <- cis[grepl("Old.*agHH", Coef), ]
cis[, Sibling := "older brother"]
cis[grepl("F", Coef), Sibling := "older sister"]
cis[, Sibling := factor(Sibling)]
library(ggplot2)
PointRange <-  geom_pointrange(aes(ymin = CI_L, ymax = CI_U),
    stat = "identity", fatten = 1.75, 
    position = position_dodge(width = .25))
g <- ggplot(data = cis, 
    aes(x = group, y = beta, group = ageLB, fill = ageLB, shape = ageLB, colour = ageLB)) + 
  PointRange + ThisTheme + facet_grid(Sibling ~ gender) +
  xlab("data") + 
  ylab("estimates") + 
  scale_x_discrete(guide = guide_axis(n.dodge = 2)) +
  labs(color  = "age lowerbound", fill = "age lowerbound", 
    shape = "age lowerbound") +
  ThisThemeEnd +
  guides(
    colour = guide_legend(title = "age lowerbound", nrow = 1),
    fill = guide_legend(title = "age lowerbound", nrow = 1),
    shape = guide_legend(title = "age lowerbound", nrow = 1)
    ) +
  geom_hline(aes(yintercept = yintercept), colour = "lightgreen")
pdf(
  "../save/OlderSiblingPlotsByAgeLBByGender.pdf"
  , width = 2*12/2.54, height = 2*8/2.54)
print(g)
whatever <- dev.off()
g
...
<details><summary>Click here to see the R script of plotting estimates on programs.</summary>

```{r plot main programs, eval = T, echo = T}
#### results <- qread("../save/DID2024_MainResults.qs")
#### results <- qread("save/DID2024_MainResults.qs")
s <- de <- 1
j <- m <- clnum <- 2
...
cis[, yintercept := 0]
cis[, gender := factor(gender, levels = genderitems)]
cis <- cis[grepl("rogram", Coef), ]
cis <- cis[grepl("10", ageLB), ]
cis[, Coef := factor(Coef)]
cis[, spec := factor(spec)]
cis[, Spec := paste(Coef, spec)]
library(ggplot2)
PointRange <-  geom_pointrange(aes(ymin = CI_L, ymax = CI_U),
    stat = "identity", fatten = 1.75, 
    position = position_dodge(width = .25))
...
M9L <- gsub(
  ...
  paste(
    ...
    sep = "&&")
  , M9L)
aghhrows <- grep("AgHH", M9L)
for (iii in 1:length(aghhrows))
  M9L[aghhrows[iii]] <- gsub("AgHH:.*?\\&", 
...
M9 <- rbind(M910[(aghhrows[2]):(aghhrows[3]-1), ], 
  ...
  M912[c((aghhrows[2]):(aghhrows[3]-1), 
    grep("^Tha", M912[, 1]):grep("^HH", M912[, 1])), ])
#### M9L <- 
...
M9 <- do.call(cbind, M9)
colnames(M9) <- c("variables", rep(paste("Spec", 1:3), 3))
bgcolor <- "#ffcc99"
kt <- kbl(M9, align = paste0("r", paste0(rep("c", ncol(M9)-1), collapse = "")), 
  row.names = F, caption = 
...
kt <- pack_rows(kt, group_label="Common specifications", grep("^Tha", M9[, 1]), grep("^HH", M9[, 1]))
kt <- scroll_box(kt, height = "800px")
kt <- footnote(kt, 
           general = "Sample of direct offspring of household heads. ",
           number = c(SpecMemo1html, SEMemoForSelectedResultsNoStarhtml)
...
PointRange <-  geom_pointrange(aes(ymin = CI_L, ymax = CI_U),
    stat = "identity", fatten = 1.75, 
    position = position_dodge(width = .25))
AddToFileName <- c("", "HeadReply", "Occ", "Combined")
AgDefShort <- c("is", "hd", "oc", "0")
for (iii in 1:length(AgDefShort)) {
  g <- ggplot(data = cis[grepl(AgDefShort[iii], agdef), ], 
      aes(x = group, y = beta, group = spec, fill = spec, shape = spec, colour = spec)) + 
    ...
    geom_hline(aes(yintercept = yintercept), colour = "lightgreen")
  pdf(
    paste0("../save/MainVsPlaceboPlotsByAgeLBByGender", AddToFileName[iii], ".pdf")
    , width = 2*12/2.54, height = 2*8/2.54)
  print(g)
  whatever <- dev.off()
  pdf(
    paste0("../draft/Figures/MainVsPlaceboPlotsByAgeLBByGender", AddToFileName[iii], ".pdf")
    , width = 2*12/2.54, height = 2*8/2.54)
  print(g)
  whatever <- dev.off()
  assign(paste0("g", AgDefShort[iii]), g)
}
```
</details>
...
```{r PlotResultsByAgeLB}
g
```

Head's reply based.

```{r PlotResultsByAgeLBhd}
ghd
```

Occupation based.

```{r PlotResultsByAgeLBocc}
goc
```

Combined.

```{r PlotResultsByAgeLBaghh0}
g0
```
...
* Verdict: Impacts found for boys of 11 years and older, even with raw DID.  
  * AgHH definition: Income source.  
  * Holds under all agHH definitions.  
...
* We need to refer to these figures in the main text that raw DID can show impacts among older boys.  
...
res2 <- Res2[grepl("di", HHtype) & grepl("is", agdef) & grepl("m.1", data)
   & grepl(2, agegroup) & !is.na(group), ]
nR <- NR2[grepl("is", agdef) & grepl("m", data) 
  & grepl("^B", inference) & grepl("di", HHtype) & grepl(2, agegroup), ]
enr0 <- unique(Enr2[grepl("is", agdef) & grepl("m", data) & grepl(2, agegroup)
...
```{r show num grades days absent full results, eval = T}
#### HTML output
library(kableExtra)
Tab <- TabGD
colnames(Tab) <- c("variables", rep(c("raw DID", "+covariates", "+HH trends"), 3))
...
for (ge in 1:3) {
  ...
  tab <- tabs2latex4(cis, informat = "sattCI", outformat = "epc", xx.yyy = T,
    AddStarIntabstarP = ADDStar, UseHTML = T)
  tab <- tab[grepl("^agHH.yr\\d|\\{agHH.yr\\d|Prog", rownames(tab)), ]
...
SepCols <- c(3, 6) 
ltb <- latextab(Tab, delimiterline = NULL, 
    hcenter = c(3, rep(1.5, ncol(Tab)-1)),
    hleft = c("\\scriptsize", rep("\\hfil\\scriptsize$", ncol(Tab)-1)), 
    hright = c("\\hfill", rep("$", ncol(Tab)-1)),
    headercolor = NULL, 
...
kt <- row_spec(kt, rep(grep("^agHH.yr.$", Tab[, "variables"]), each = 3)+0:2, bold = F, 
  color = "black", background = "#87cefa")
...
```{r plot grade days, eval = T, echo = T}
ii <- jj <- s <- 1
j <- m <- clnum <- 2
rr <- 3
cis <- NULL
for (ii in 1:2) {
  for (gd in 1:5) {
    #gd 1: NumGrade 1999 enrollers, 2: NumGrade complete panel enrollers, 
    #    3: DaysAbsent 1999 enrollers, 4: DaysAbsent 2002 cross sectional OLS
    #    5: DaysAbsent 2002 cross sectional OLS
    # Placebo is estimated only for NumGrades. No DaysAbsent data collected in 2006.
    if (gd > 2 & ii == 2) next
    # For gd == 4, rr = 1 (1999, July-August, asked in Sep-Oct) covers 
    # For gd == 5, rr = 2 (2002, July-August, asked in Sep-Oct) covers 
    # 2 months of planting season absenteeism in a 3 month recall question.
    if (gd == 4) rr <- 1
    if (gd == 5) rr <- 2
        for (yy in 1:2) { 
    ...
    for (ge in c(3, 1, 2)) {
      ...
      cis0[, outcome := c(rep(c("Grades", "Days"), each = 2), "Days")[gd]]
      cis0[, year := c(1999, 2002, NA)[rr]]
      cis0[, sample := c("incomplete panel", "complete panel", "incomplete panel",
        "cross section 1999", "cross section 2002")[gd]]
...
cis[, data := factor(data)]
cis[, gender := factor(gender, levels = genderitems)]
cis[, outcomes := outcome]
cis[grepl("pla", data), outcomes := "Grades, placebo"]
cis[, outcomes := factor(outcomes, levels = c("Grades", "Grades, placebo", "Days"))]
cis[, outcome := factor(outcome, levels = c("Grades", "Days"))]
cis[, sample := factor(sample)]
cis[, sample := factor(sample, labels = gsub("panel", "", levels(sample)))]
cis[, sample := factor(sample, labels = gsub("cross section ", "", levels(sample)))]
library(ggplot2)
PointRange <-  geom_pointrange(aes(ymin = CI_L, ymax = CI_U),
    stat = "identity", fatten = 1.75, 
    position = position_dodge(width = .25))
g <- ggplot(data = cis, 
    aes(x = sample, y = beta, group = spec, fill = spec, shape = spec, colour = spec)) + 
  PointRange + ThisTheme + facet_grid(outcomes ~ gender, scales = "free_y") +
  xlab("data") + 
  ylab("estimates") + 
  scale_x_discrete(guide = guide_axis(n.dodge = 2)) +
  labs(color  = "regression specifications", fill = "regression specifications", 
