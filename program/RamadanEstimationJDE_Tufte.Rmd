...
<a name="EstimationOtherSchoolingOutcomes">Impacts on number of grade progression and days absent.</a>
...
Agricultural household definition is based on `r Aghh.defs[3]`(`r aghh.defs[3]`)  

```{r show main table, eval = T, warning = F}
#### Uses tabs2latex4
Enr <- qread("../save/TabulatedAllResultsEnr.qs")
NR <- qread("../save/TabulatedAllResultsNR.qs")
#### results <- qread("save/DID2024_MainResults.qs")
...
* We use head's reply for the agricultural household definition.  
   * This follows from the questionnaire of original IFPRI data set.  
...
* Impacts are strongest among the boys.  
...
```
```{r read nonmuslim flood results, eval = T, warning = F}
resultsM <- qread("../save/DID2024_NonMuslimGenderResults.qs")
resultsF <- qread("../save/DID2024_FloodGenderResults.qs")
...
* Girls in flooded areas decrease enrollment rates in non-agricultural househlds (they increase in agricultural households although the $p$ values are not small).  
...
* Grade progression impact estimates are negative. Exam-harvest overlap delays progression by .46 years (`r round((c(.46))*12, 2)` months).   
...
We use <span style='color:blue; font-size:100%; font-weight:600'> [glht](https://search.r-project.org/CRAN/refmans/multcomp/html/glht.html)  </span> of <a href='https://cran.r-project.org/web/packages/multcomp/index.html'>multcomp package</a> for linear hypothesis testing.  
...
* Attrition rate is higher among non-ag HHs. Given the smaller enrollment rate changes among them, this has a potential to understate the impacts. We also note that attrited households between ag and nonag differ in terms of head's spouse education level. However, we see in below that, when we estimate the attrition selection with the sample of attrited and non-attrited households, the attrition can be considered mostly as random.  
* Based on linear probability estimation, attrition seems random between agHHs and nonag HHs at the household level. There is an indication that smaller land holders of agHHs may be more prone to attrit at the rate of 3% per one acre reduction in landholding.  
* The implied magnitude of correlation between landholding and attrition is small given the mean landholding of agHHs is 1.46 acre. In addition, it is not easy to guess the direction of the bias it gives to enrollment rates: When smaller landholding has smaller labor demand for children that results in higher enrollment rates, their attrition can understate enrollment rates. When smaller landholders, or less wealthy households, stop schooling early that results in lower enrollment rates, their attrition can overstate enrollment rates. Given the small magnitude of possible impacts that may potentially cancel out each other and a mildly small $p$ value associated with it, we restrict our attention to the complete panel portion of the data. We also note that we include landholding as a covariate in the main estimation.  
...
reatab <- data.frame(cbind(
  group = rep(paste0("\\mbox{\\scriptsize ", 
    c("non-goers 1999","non-goers 2002","drop outs 2002"), "}"), 4), 
  formatC(reatab[, -ncol(reatab)], digits = 2, format = "f"), total = reatab[, ncol(reatab)]))
colnames(reatab)[-c(1, ncol(reatab))] <- cn
reatab <- cbind(quartile = repseq(1:4, 3), reatab)
#### LaTeX table
write.tablev(latextab(as.matrix(reatab),
  hleft = "\\scriptsize\\hfil$", 
  hcenter = c(1, 2, .8, rep(1.5, 2), 2, rep(.8, 4)), 
  hright = "$", headercolor = "gray90", delimiterline = NULL), 
  "../save/reasons.tex", colnamestrue = F)
...
reatab <- data.frame(cbind(
  Group = rep(paste0("\\mbox{\\scriptsize ", 
    c("Non-goers 1999","Non-goers 2002","Drop outs 2002"), "}"), 2), 
  formatC(reatab[, -ncol(reatab)], digits = 2, format = "f"), total = reatab[, ncol(reatab)]))
colnames(reatab)[-c(1, ncol(reatab))] <- cn
reatab <- cbind("HH type" = repseq(c("ag", "non-ag"), 3), reatab)
#### LaTeX table
write.tablev(latextab(as.matrix(reatab),
  hleft = "\\scriptsize\\hfil$", 
  hcenter = c(1, 2, .8, rep(1.5, 2), 2, rep(.8, 4)), 
  hright = "$", headercolor = "gray90", delimiterline = NULL), 
  "../save/reasons_ag.tex", colnamestrue = F)
#### HTML table
...
kt <- kable(reatab, align = paste0("r", paste0(rep("c", ncol(reatab)-1), collapse = "")), 
  caption =   "Reasons stop going to school")
...
agp <- cbind(type = c("Non agricultural households", "Agricultural households", "Total"), agp)
#### LaTeX table
write.tablev(latextab(as.matrix(agp),hleft = "\\scriptsize\\hfil$", 
  hcenter = c(rep(1.5, ncol(agp))), hright = "$", headercolor = "gray90", delimiterline = NULL), 
  "../save/tab_agpc4.tex", colnamestrue = F)
#### HTML table
...
kt <- footnote(kt, 
           general = "Sample of direct offspring of household heads. q1 - q4 indicate quartiles 1 - 4, and the numbers are percentages of row totals. Totals are number of households. "
           )
kt
```
