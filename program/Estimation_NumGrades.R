...
#### Educa07 (grade level in numbers, 1, 2,... ) is a variable created by referring to class07.
#### Corresponding variable in 1999-2002 is NumGrades
zEp.2002[survey==2006, NumGrades := Educa07]
zEp.2002[, dNumGrades := diff(NumGrades), by = uniquid]
zEp.2002[survey==2002, dNumGrades := NA]
#### Certain variations are found in grade progression
addmargins(table0(zEp.2002[Age >= 10 & Age <= 18 & survey==2006, .(Age, dNumGrades)]))
addmargins(table0(zEp.2002[Age >= 10 & Age <= 18, .(survey, NumGrades)]))
#### Correct negative grade progression: 
####  2002=reads in class 5, 2006=completes class 4=> 0 grade progression,
####  but this case is computed as -1 year of grade progression
zEp.2002[uniquid %in% uniquid[dNumGrades<0], NumGrades := NumGrades[1], by = uniquid]
zEp.2002[, dNumGrades := NULL]
...
####for (ii in 1) {
  ...
  for (jj in 1:length(zSobj)) {
    ...
    table(z01[, .(ss1, EnrollerStatus)])
    table(z01[, .(ss2, EnrollerStatus)]) 
    # EnrollerStatus = others: 2 cases
    #   1. ss1 = 0, ss2 = 1: Late enrollers
    #   2. ss1 = 0, ss2 = 0: Never enrollers
    z01[grepl(0, ss1) & grepl(1, ss2), EnrollerStatus := "late"]
    z01[grepl(0, ss1) & grepl(0, ss2), EnrollerStatus := "never"]
    z01[, EnrollerStatus := factor(EnrollerStatus, levels = 
      c("initial", "all time", "late", "never"))]
    table(z01[, .(ss2, EnrollerStatus)])
