...
for (ii in 1:2) {
  for (gd in 1:4) {
    ...
    #    3: DaysAbsent 1999 enrollers, 4: DaysAbsent 1999 cross sectional OLS
    Tab0 <- NULL
    # Placebo is estimated only for NumGrades. No DaysAbsent data collected in 2006.
    if (gd > 2 & ii == 2) next
    for (yy in 1) { # if gd == 4, do yy = 1, 2 else do yy = 1 only
      for (ge in c(3, 1, 2)) {
        if (gd < 3) 
        ...
        if (gd == 4) nR <- nR0[CrossSecEnrollers == c("AtLeastOnce", "BothPeriods")[rr], ] else nR <- nR0
        nr <- rbind(
...
r2n2 <- c("R2", "N: agHH", "N", 
  paste0("mean of ", rep(c("control in ", "treated in "), 2), rep(c(2002, 2006), each = 2)))
r2ncross <- c("R2", "N: agHH", "N", paste0("mean of ", c("control in ", "treated in "), 1999))
rnm11 <- c(" ", rn11, r2n1)
rnm12 <- c(" ", rn12, r2n2)
rnm21 <- c(" ", rn21, r2n1)
rnm22 <- c(" ", rn22, r2n2)
rnm31 <- c(" ", rn31, r2n1)
rnm41 <- c(" ", rn41, r2ncross)
Tab <- as.matrix(cbind(covariates = 
  c(rnm11, rnm21, rnm31, rnm41, rnm12, rnm22)
  , Tab))
Tab <- rbind(Tab, 
  "Thana fixed trends" = c("Thana fixed trends", rep(c("", "Y", "Y"), 3)),
  "HH fixed trends" = c("HH fixed trends", rep(c("", "", "Y"), 3)))
...
kt <- pack_rows(kt, group_label="Days absent: Students enrolled in 1999 and 2002, cross section OLS of 1999", 
...
* Placebo estimation using 2002 cohort shows children from agricultural households tend to progress more slowly by `r round(c(.17, .23)/4, 2)` year per annum (`r round((c(.17, .23)/4)*12, 2)` months), which are roughly half in size of delayed progression than the 1999 impacts, although the $p$ values are not small.  
