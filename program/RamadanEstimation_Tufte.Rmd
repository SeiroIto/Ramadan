...
rn[grepl("^CI|^p", rn)] <- ""
rn2 <- c("R2", "N: agHH", "N", 
  paste0("mean of ", rep(c("control in ", "treated in "), each = 2), rep(c(1999, 2002), 2)))
rnm <- rep(c(" ", rn, rn2), 3)
...
for (jj in 1:2) {
  ...
  rn[grepl("^CI|^p", rn)] <- ""
  rn2 <- c("R2", "N: agHH", "N", 
    paste0("mean of ", rep(c("control in ", "treated in "), each = 2), rep(c(2002, 2006), 2)))
  rnm <- rep(c(" ", rn, rn2), 3)
  ####  replace variable names, deleting std error names
  for (k in 1:nrow(sbt)){
    if (any(grepl(sbt[k, "org"], rnm))) 
...
NR <- qread("../save/GenderGradeDaysAbsentTabulatedNR2024.qs")
Enr <- qread("../save/GenderGradeDaysAbsentTabulatedEnr2024.qs")
nR0 <- unique(NR[agelb == (10:12)[1] & grepl(0, agdef) & grepl("^d", HHtype), ])
Enr0 <- unique(Enr[agelb == (10:12)[1] & grepl(0, agdef) & grepl("^d", HHtype), ])
Enr0[, EnRate := formatC(EnRate, digits = 4, format = "f")]
ii <- jj <- en <- s <- m <-  1
j <- clnum <- rr <- 2
#### enrs <- c("initial", "all time", "others")[en]
      ...
      for (ge in c(3, 1, 2)) {
        ...
        enr <- matrix(
         enr0[file == gdfiles[gd] & grepl(genderregexpr[ge], gender) &
            CrossSecYear == c(1999, 2002)[yy], ][
           order(agHH, tee), EnRate], byrow = F, nrow = enronumrows)
        enr <- data.table(enr[, rep(1, 3)])
        nr <- data.table(nr)
        nr[2:3, c(1, 3) := ""]
        enr[, c(1, 3) := ""]
        tab2 <- rbindlist(list(tab0, rbind(nr, enr)), use.names = F)
        Tab00 <- cbind(Tab00, tab2)
      } # ge
      Tab00 <- rbind(t(matrix((gd-1)*9+(yy-1)*9+(ii-1)*45+1:ncol(Tab00))), Tab00, use.names = F) 
      Tab0 <- rbind(Tab0, Tab00)
    } # yy
    Tab <- rbind(Tab, Tab0)
  } # gd
...
* Stipend and non-stipend programs may improve grade progression among girls, and they may improve daily attendance among boys. While it is unclear why they only affect atendance but not progression among boys, girls may attend schools constantly with or without them because they are less engaged in remunerative work. Lower opportunity costs of schooling among girls may also allow a stronger responce to programs in terms of progression.  
* One possible interpretation is: Boys respond in terms of daily attendance/absence, but not in terms of progression. Opposite is true for girls.  
...
* While we do not have information to support the claim, it is possible that these children who progressed to the next grade may have opted not to work in the field during the planting period of 2000.   
...
write.tablev(ltb2
  ,  "MainGenderAgeGroup2Results.tex"
  ,  colnamestrue = F)
#### HTML outputs
mrtab <- rbind(mrtab, 
  "Thana fixed trends" = c("Thana fixed trends", rep(c("", "Y", "Y"), 3)),
