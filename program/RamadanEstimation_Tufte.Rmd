...
kt <- row_spec(kt, rep(grep("^agHH.yr.$", Tab[, "variables"]), each = 3)+0:2, bold = T, 
  color = "black", background = "#87cefa")
kt <- pack_rows(kt, group_label="Boys+Girls", grep("^agH", rnm)[1]-1, grep("^agH", rnm)[1]+2)
kt <- pack_rows(kt, group_label="Boys", grep("^agH", rnm)[2]-1, grep("^agH", rnm)[2]+2)
kt <- pack_rows(kt, group_label="Girls", grep("^agH", rnm)[3]-1, grep("^agH", rnm)[3]+2)
...
Tab <- as.matrix(cbind(covariates = 
  c(rnm11, rnm21, rnm31, rnm41, rnm42, rnm12, rnm22)
  , Tab))
Tab <- rbind(Tab, 
  "Thana fixed trends" = c("Thana fixed trends", rep(c("", "Y", "Y"), 3)),
  "HH fixed trends" = c("HH fixed trends", rep(c("", "", "Y"), 3)))
...
* Grade progression impacts are negative and delays progression by .35-40 years in a three year period (`r round(c(.35, .40)/3, 2)` per annum or `r round((c(.35, .4)/4)*12, 2)` months). Only the larger sample gives small $p$ values. Impacts on boys are larger in magnitude yet the sample size is too small, because we must condition on children who are enrolled in 1999.  
...
* In contrast to this, cross sectional estimates in 2000 (using enrollers of 1999 and 2002) show a zero increase for children of agricultural households. In 1999, children from agricultural households benefited from the exam preponement.  While we do not have information to support the claim, it is possible that these children who progressed to the next grade may have opted not to work in the field during the planting period of 2000.   
* Placebo estimation using 2002 cohort shows children from agricultural households tend to progress more slowly by `r round(c(.17, .23)/4, 2)` year per annum (`r round((c(.17, .23)/4)*12, 2)` months), which are roughly half in size of the 1999 impacts, although the $p$ values for placebo estimation are around 10% and are not as small as 1999 impact estimates.  
