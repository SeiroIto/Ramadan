...
ltb <- c(
  ...
  ltb[(aghhrows[1]-1):(aghhrows[2]-2)],
  "&\\multicolumn{11}{c}{\\scriptsize B. Secondary school ages}\\\\",
...
for (iii in 1:(length(paneltitles)-1)) {
  ...
  ltb <- c(
      ...
      paste0("&\\multicolumn{11}{c}{", LETTERS[iii], ". ", paneltitles[iii], "}\\\\"),
      ltb[(aghhrows[iii]-1):length(ltb)]
    )
}
ltb <- c(
    ltb[1:(grep("^Than", ltb)-2)],
    "&\\multicolumn{11}{l}{Common specifications}\\\\",
    ltb[(grep("^Than", ltb)-1):length(ltb)]
  )
ltb <- gsub("CI\\$.*\\$", "", ltb)
ltb <- gsub("p\\$.*\\$", "", ltb)
ltb <- gsub("(^\\\\cline.*$)", "\\1[-1ex]", ltb)
...
for (iii in 1:(length(paneltitles)-1)) {
  ...
  ltb <- c(
      ltb[1:(aghhrows[iii]-2)],
      paste0("&\\multicolumn{7}{c}{", LETTERS[iii], ". ", paneltitles[iii], "}\\\\"),
      ltb[(aghhrows[iii]-1):length(ltb)]
    )
}
ltb <- c(
...
kt <- pack_rows(kt, group_label="Grade progression: Placebo estimation of 1999 cohort enrolled in 2002", 
  aghhrows[7]-1, grep("^Tha", Tab[, 1])-1)
kt <- pack_rows(kt, group_label="Common specifications", 
  grep("^Tha", Tab[, 1]), grep("^HH", Tab[, 1]))
kt <- scroll_box(kt, height = "800px")
kt <- footnote(kt, 
           general = "Sample of direct offspring of household heads. ",
           number = c(SpecMemo1, SEMemoForSelectedResults)
