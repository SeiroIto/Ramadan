...
kt <- row_spec(kt, rep(grep("Agri", Tab[, "variables"]), each = 3)+0:2, bold = F, 
  color = "black", background = "#87cefa")
kt <- pack_rows(kt, group_label="Boys", grep("Agri", rn)[1], grep(" Non.*ag", rn)[1]+2)
kt <- pack_rows(kt, group_label="Girls", grep("Agri", rn)[2], grep(" Non.*ag", rn)[2]+2)
kt <- pack_rows(kt, group_label="Boys+Girls", grep("Agri", rn)[3], grep(" Non.*ag", rn)[3]+2)
kt <- scroll_box(kt, height = "800px")
kt <- footnote(kt, 
           general = "Sample of direct offspring of household heads. ",
           number = c(SpecMemo1, SEMemoForSelectedResults)
