...
zEp.2002 <- qread("../save/zEp2002.qs")
zEm.1999[, agHH0 :=  as.numeric(agHH0 > 0)]
zEp.2002[, agHH0 :=  as.numeric(agHH0 > 0)]
zEp.1999[, AgeIn1999 := Age[survey == 2002] - 3, by = uniquid]
zEp.1999[, AgeIn2002 := Age[survey == 2002], by = uniquid]
...
for (ii in 1:length(samples)) {
  ...
  for (jj in 1) {
    ...
    z01[, ss := cumsum(schoolp), by = uniquid]
    z01[, ss := ss[2], by = uniquid]
    # Choose enrollment status: 1=at least once, 2=both rounds
    for (rr in 1:2) {
      # Choose year: 1=1999, 2=2002
      for (yy in 1:2) {
            ...
            for (j in 2) {
              ...
              for (ge in 1:3) {
                ...
                for (m in 1:length(aghh.defs)) {
                  ...
                  results0[[jj]][[yy]][[r]][[s]][[j]][[ge]][[m]] <- resul
...
#### inference is not given unique level clnum. Each k spec has ciLZ and ciBRL.
#### results[[ii]][[jj]][[rr]][[s]][[j]][[ge]][[m]][[k]]
#### DAbs cross section resultsDC[[ii]][[jj]][[rr]][[s]][[j]][[ge]][[m]][[k]], yy = 1, 2
...
for (gd in 1:4) {
  for (ii in 1) {
    ...
    for (jj in 1:length(zSobj)) {
      thisdata <- zSobj[[jj]]
      for (yy in 1:yylast[gd]) { # if gd == 4, do yy = 1, 2 else do yy = 1 only
        for (s in 1:3) {
          for (j in 1:2) {
            for (ge in 1:3) {
              for (m in 1:4) {
                for (clnum in 1:2) {
                  if (gd <= 2) 
                    ...
                    estGDs1 <- resultsDC[[ii]][[jj]][[yy]][[s]][[j]][[ge]][[m]]
                  if (gd == 4) {
                      if (j == 1) next
                      if (clnum == 1) next
  ...
  for (yy in 1) { # if gd == 4, do yy = 1, 2 else do yy = 1 only
    for (ge in c(3, 1, 2)) {
      if (gd < 3) 
        ...
        cis0 <- lapply(resultsDC[[ii]][[jj]][[yy]][[s]][[j]][[ge]][[m]], "[[", "ciBRL")
      cis0 <- lapply(cis0, data.table)
      cis <- lapply(cis0, function(x) x[, df := NULL])
      cis <- lapply(cis, function(x) {
       dm <- data.frame(x[, -1])
...
kt <- pack_rows(kt, group_label="Grade progression: Enrolled in 1999", 
  aghhrows[1], aghhrows[2]-1)
kt <- pack_rows(kt, group_label="Grade progression: Enrolled in 1999 and 2002", 
  aghhrows[2], aghhrows[3]-1)
kt <- pack_rows(kt, group_label="Days absent: Enrolled in 1999 and 2002", 
  aghhrows[3], aghhrows[4]-1)
kt <- pack_rows(kt, group_label="Days absent: Enrolled in 1999, cross section OLS", 
  aghhrows[4], grep("^Tha", Tab[, 1])-1)
